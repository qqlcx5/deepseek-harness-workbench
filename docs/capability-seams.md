...
| `ctx.memory` | `seam` | [`memory`](../packages/memory/memory) | [`memory-tencentdb`](../packages/memory/memory-tencentdb) | [`memory-session`](../packages/memory/memory-session) | - | Memory providers register into one ctx.memory seam; memory-session recalls into each turn and archives finished turns. |
